...
> 最近更新：2026-08-05 · 当前阶段：**P0 关闭（视频生成项延期至 P2：Seedance 开通需 ¥200 底额，前期全图卡测试）；可进入 P1**
...
### 1.1 大纲生成
- [ ] 大纲 Prompt（主题句/结构段落/目标时长/受众 + **style 字段由 LLM 按选题自行设计，固化为项目级常量注入后续所有 prompt**——"LLM 选型，项目内锁定"，P6 Skill 选择器的雏形，2026-08-05 决策）
- [ ] 大纲落库（script.json + projects 表）
...
- [ ] 参考图链：首镜图作为后续镜头参考图传入 Seedream（无人工确认环节，直接用首张产出图）
...
### P1 验收标准
- [ ] 3 个不同选题均产出结构合法的分镜包（script.json + shots/*.png）
- [ ] 分镜图质量人工抽看评估（确认交互延后，改为产出后抽看记录通过率作基准数据）
- [ ] 单选题成本 ≤¥3，本周 ≤¥20
