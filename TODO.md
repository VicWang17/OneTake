...
> 最近更新：2026-08-06 · 当前阶段：**P2 端到端 v1 达成（里程碑：选题一键到成片）；可进入 P3**
...
| P4 服务化+调度（W5） | 模型服务层 + 任务调度器 | ≈¥0 | ⬜ | |
| P5 观测+数据（W6） | 可观测性 + 数据链路 | ≈¥0 | ⬜ | |
| P6 Skills/记忆（W7） | 2 个 Skill 包 + 风格跨项目复用 | ≤¥30 | ⬜ | |
| P7 质检成片（W8） | VLM 质检 + 3 条成片产出（不发布） | ≤¥40 | ⬜ | |
| P8 开源交付（W9） | 可复现仓库 + 演示视频 | ≈¥0 | ⬜ | |
...
> ✅ 熔断已于 2026-08-06 恢复 ¥15（手动调试期临时放宽已撤销，3.1 验收时复核熔断仍生效）
...
### 3.3 成本报表 ✅ 2026-08-07
- [x] `report` 完整版：项目总览（镜头/总成本/单镜成本）+ 环节×模型 + 档位拆分 + 命中率 + 节省估算
- [x] `report -p` 单条成本明细：环节占比 + 对标验收线（彩虹二号 ¥7.29 ≤¥8 ✅，video 占比 77.8% 实证 PRD 判断）

### 3.4 失败降级 ✅ 2026-08-07
- [x] 降级链：网关 FALLBACKS 表（DeepSeek→Qwen 跨供应商、Seedream 4.0→4.5 跨版本、Seedance 2.0↔fast 跨档互备）；降级结果不写 idem_key 防污染缓存
- [x] 互备演练：LLM 断连模拟 → Qwen 接管（¥0.0001）；视频标准档注入故障 → fast 接管（¥0.71）；degraded_from 全落库可查；撞出 Qwen json 模式怪癖（DEVLOG 020）
...
## Phase 6 · Skills 与记忆系统（W7，预算 ≤¥30）✅ 已完成（2026-08-10，实际 <¥0.2）

目标：创作方法论资产化。时间不足时本阶段可压缩为演示级。

### 6.1 Skill 系统
- [x] Skill YAML Schema + 加载器（校验 + upsert 注册）+ 注册表
- [x] Skill `knowledge_explainer_v1.yaml`（知识科普解说）
- [x] Skill `movie_commentary_v1.yaml`（影视片段解说）
- [x] Skill 选择器：LLM 按选题匹配（三选题实测全对）；`--skill` 强制指定；无匹配回退自决

### 6.2 记忆系统
- [x] memories 表启用：profile / episode 两类
- [x] 画像记忆：`memory add` 手动通路（确认节点意见自动归纳待有真实确认数据后接）
- [x] 经验记忆：`memory extract` 从失败记录 LLM 归纳（实测产出 3 条）
- [x] 记忆合并：LLM 决策 add/merge/contradict（实测归并正确，置信度 0.5→0.65）
- [x] 记忆注入：Top-K + 置信度排序进大纲 prompt（实测注入且有题材分辨力）

### 6.3 批量验证（文本层，按用户决策不做全量出图）
- [x] 批量模式：同一 Skill 连出 2 个分镜包（含羞草/银行卡），结构完全一致、锚点各自贴合选题
- [x] 风格复用演示：加记忆"标题必须带数字" → 新大纲标题"…？3个真相"——因果明确

### P6 验收标准
- [x] 风格复用演示（记忆注入因果验证，见上；录屏留证待 P8 演示视频统一录）
- [x] 批量文本层成本 <¥0.2（全量出片批量验证并入 P7；stats 队列可见）
- [x] Skill 切换风格差异肉眼可辨（千与千寻同题双 Skill 大纲对比）

---

## Phase 7 · 质检与成片闭环（W8，预算 ≤¥40）✅ 修订版完成（2026-08-10，实际 <¥0.1）

目标：**修订为"做一次全链路质量检测"**（2026-08-10 用户决策）；720p 成片档渲染与 3 条成片产出推迟到 P8 或按需。

### 7.1 VLM 质检
- [x] 抽帧工具：每条镜头视频抽 1/3/5s 三帧（覆盖"开头好后面崩"）
- [x] `qwen3-vl-flash` 双维度评分（语义一致性/画面质量，各 1–5）——网关新增 vl 任务类型
- [x] 低分自动重生成逻辑（任一 <3，≤2 次，第 2 次附失败原因；重生成走文生视频避免首帧锚定延续错误构图）
- [x] 仍不通过 → 标记人工介入
- [x] 质检结果进 events（eval 类）+ quality.json 报表（一次通过率）
- [x] **评分器区分度验证**：刻意错配样本 → semantic=1 判不合格（DEVLOG 023：评估器本身需要被评估）

### 7.2/7.3 推迟项
- [-] 成片档 720p 渲染、3 条成片产出、演示物料——推迟到 P8（演示视频制作时按需产出）

### P7 验收标准（修订口径）
- [x] 全链路质检真实跑一次：彩虹二号 8/8 通过、一次通过率 100%（质量真实，评分器已验证有区分度）
- [x] 质检真实成本 <¥0.1（VLM 走百炼免费额度口径 ≈¥0.01）
- [-] 重生成回路实战触发——本次无真实低分镜头，逻辑已备未实战（诚实记录，待真实低分出现验证）
- [ ] 终稿 VLM 一次通过率 ≥80%
...
- [ ] README：项目定位、五层架构图、demo 链接、成本数据、快速开始（30 分钟可复现验证一遍）
...
| 时间不足（课业冲突） | 砍 Scope 顺序：P2 > P3 > **P4（服务化+调度）** > P5 > P6 > P7 |
...
- 2026-08-04 **v2.0 对齐 prd.md**：新增 P4（模型服务化+任务调度，W5）与 P5（可观测性+数据链路，W6）两个阶段；原 P4–P6 顺延为 P6–P8；P0 新增供应商实测对比任务（0.6）；数据层 schema 一次到位（含 jobs/events/model_perf_daily）
