...
| P7 评测发布（W8） | VLM 质检 + 3 条成片发布 B 站 | ≤¥60 | ⬜ | |
...
- [x] ✅ BGM 方案：**Pixabay Music**（免费商用、无需署名）；FMA 须逐曲核 CC；国内素材站不作首选
- [x] ✅ B 站 AIGC 规则：投稿必须勾选「该视频使用人工智能合成技术」创作声明（2025-09 起法规义务）
...
- [ ] 火山引擎：实名认证、开通 Seedream + Seedance、确认免费额度到账；**首充 ¥20–50 即可**（P0 现金需求仅约 ¥2–4 的分镜图费用，¥100 是全项目口径的省事值，花完再补）；确认 Seedream 按张计费是否在 token 免费额度抵扣范围内；控制台查看是否可开"超额后付费"（可开则预充值可进一步省掉）；设消费告警、关自动充值
- [ ] DeepSeek：注册、确认赠送余额到账；**暂不充值**，欠费再说；设消费告警
- [ ] 阿里云百炼：实名认证、开通 qwen3.7-flash + qwen3-vl-flash（华北2/北京）；**不用充值**（100 万 token/模型免费额度足够）
- [ ] 可灵开放平台：注册、控制台确认 API 侧是否有赠点；**无赠点再充 ¥50**（够跑十几条 5s 对比样片）
...
- [ ] Seedance（1.5 pro 样片模式）与可灵（2.5 Turbo）各跑一条 5s 样片
...
- [ ] 聚合报告真实产出（ Seedance vs 可灵对比，复验 P0 结论）
...
## Phase 7 · 评测与发布闭环（W8，预算 ≤¥60）

目标：质量自评 + 真实发布，把「做过」升级为「用过、有数据」。
...
### 7.3 发布物料与发布
- [ ] 发布物料：候选标题 ×3 + 简介 + 标签 + 封面合成
- [ ] 发布 3 条视频到 B 站，**勾选「该视频使用人工智能合成技术」创作声明**，标题注明全流程自研 AI 管线生成
- [ ] 72 小时播放/完播数据回写（publish 事件 → 数据链路 → skill 效果表）
...
| 触发条件 | 预案 |
|---|---|
| Seedance 接口 2 天调不通 | 立即切可灵，不阻塞（P0 红线） |
...
| 视频生成单价高于 v1.1 测算（已确认 6–8 倍） | 草稿档 1.5 pro 样片模式或可灵 2.5 Turbo；P0 实测后回填成本目标 |
| 某 Phase 预算超支 | 后续阶段降级：P6/P7 压缩为演示级 |
| 时间不足（课业冲突） | 砍 Scope 顺序：P2 > 1 条发布 > P3 > **P4（服务化+调度，剪映 JD 底线）** > P5 > P6 |
