...
- **AI 创作工具的产品形态**：Agent 创作助手（长期记忆）+ Skills 技能库 + 多模型聚合调度——对齐 Updream 类一站式 AI 视频创作平台的核心链路
- **AI 基础设施的工程形态**：模型服务化（注册表/灰度/熔断）、任务调度、可观测性、数据链路——把创作管线跑在一套迷你但完整的 AI Infra 上
...
| 文件 | 内容 |
|---|---|
| `prd.md` | 完整 PRD + 技术设计文档（v2.1 活文档），含架构演进记录与设计问答库（附录 C） |
